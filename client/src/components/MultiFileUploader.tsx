...
import { CategoryTrafficData } from '../types';
...
interface FileCategories {
  arl: FileCategory;
  overall: FileCategory;
  withoutArl: FileCategory;
}

const MultiFileUploader = ({ onFilesUploaded }: MultiFileUploaderProps) => {
  ...
  const categorizeFile = (file: File): { category: keyof FileCategories; type: 'rpm' | 'rps' } | null => {
    ...
    if (filename.startsWith('arl_')) {
      return {
        category: 'arl',
        type: filename.includes('rpm') ? 'rpm' : 'rps'
      };
    } else if (filename.startsWith('overall_')) {
      return {
        category: 'overall',
        type: filename.includes('rpm') ? 'rpm' : 'rps'
      };
  ...
  const processFiles = async (newFiles: File[]) => {
    ...
    for (const file of newFiles) {
      ...
      if (!categorization) {
        setError(`Invalid file: ${file.name}. File name must start with 'ARL_', 'Overall_', or 'Without_ARL_' and end with 'RPM' or 'RPS'.`);
        hasError = true;
        break;
      }
  ...
  const checkAndProcessFiles = async (fileData: FileCategories) => {
    const allCategories = Object.entries(fileData);
    const completeCategories: { [key: string]: { rpm: string; rps: string } } = {};
    
    for (const [category, files] of allCategories) {
    ...
    if (Object.keys(completeCategories).length > 0) {
      onFilesUploaded(completeCategories as any);
    }
  };
  ...
  const getFileStatus = (category: keyof FileCategories) => {
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        >
          ...
          <div className="text-center">
            ...
            <h3 className="font-medium mb-1">Upload Traffic Log Files</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          {(['arl', 'overall', 'withoutArl'] as const).map((category) => (
            <div
              key={category}
            ...
            >
              <h4 className="font-medium mb-2 capitalize">
                {category.replace(/([A-Z])/g, ' $1').trim()} Files
              </h4>
